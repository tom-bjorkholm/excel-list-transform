...
def test_store_col_split_name(capsys,  # pylint: disable=too-many-arguments
                              row, col, val, sr, nrow):
...
@pytest.mark.parametrize('ind, merg, msg',
                         [([{'x': 'a', 'y': 'b', 'z': 'c'},
                           ...
                           's3_merge_columns: no column named "q" in data')])
...
@pytest.mark.parametrize('nam, msg',
                         [([{'column': 'y', 'name': 'One'},
                            {'column': 'q', 'name': 'Zwei'}],
                           's5_rename_columns: no column named "q" in data')])
...
@pytest.mark.parametrize('ins, msg',
                         [([{'column': 'p', 'name': 'One', 'value': None},
                           ...
                           's6_insert_columns: column "x" already exists')])
@pytest.mark.parametrize('ind',
                         [[{'x': 'a', 'y': 'b', 'z': 'c'},
                           {'x': 'd', 'y': 'e', 'z': 'f'}],
                          [{'x': 'a', 'y': 'b', 'z': 'c'},
...
def test_rewrite_columns_nok_name(capsys, ind, spec):
    ...
    out, err = capsys.readouterr()
    assert 's7_rewrite_columns: no column named "q" in data row' in err
    assert '' == out
...
def get_test_data_name(written_result: bool) -> DataToUseName:
    ...
    if written_result:
        ...
        result = [{'q': 'h', 'the new': 'x', 'Second': 'd', 'x': 'a b'},
                  {'q': None, 'the new': 'x', 'Second': 'g', 'x': 'e f'}]
    ret = DataToUseName(indata=[{'x': 'a', 'y': 'b c', 'z': 'd'},
                                {'x': 'e', 'y': 'f', 'z': 'g'}],
...
def test_transform_data_ok_name(capsys):
    ...
    cfg.s3_merge_columns = test_data.merge_cols
    cfg.s5_rename_columns = test_data.rename_cols
    cfg.s6_insert_columns = test_data.insert_cols
    cfg.s7_rewrite_columns = test_data.rewrite_cols
...
@pytest.mark.smoke
def test_rfmt_nmd_files_xl2cs_name(capsys):
    ...
    cfg.s6_insert_columns = test_data.insert_cols
    cfg.s7_rewrite_columns = test_data.rewrite_cols
    cfg.s8_column_order = test_data.column_order
...
@pytest.mark.smoke
def test_rfmt_nmd_files_cs2xl_name(capsys):
