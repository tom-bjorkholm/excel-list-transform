...
def read_csv_named_use_num(filename: str, dialect: type[Dialect],
    ...
    data = read_csv_num(filename=filename, dialect=dialect,
                        max_column_read=max_column_read)
    return named_cols_from_num_cols(data=data, filename=filename)


def read_csv_named(filename: str, dialect: type[Dialect],
                   max_column_read: int) -> NameData:
...
def write_csv_num(data: NumData,
                  filename: str, dialect: type[Dialect]) -> None:
    """Write a csv file in specified dialect."""
    with open(file=filename, mode="w", encoding='utf8', newline='') as cfile:
        cwriter = writer(cfile, dialect=dialect)
        for row in data:
            cwriter.writerow(row)


def write_csv_named(data: NameData,
                    filename: str, dialect: type[Dialect],
